...
protected:
    void send_call(int call_id, emb::contiguous_buffer<unsigned char> buffer) const;
...
public:
    ...
    auto call(Args... args) {
        ...
        typename types::arguments_tuple payload{args...};

        if (auto size = ser::serialize(payload, buffer_); size >= 0) {
            send_call(static_cast<int>(Id), {buffer_.data(), size});
        }
